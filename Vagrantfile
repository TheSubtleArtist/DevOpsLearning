...
###   NETWORK VARIABLES   ###
#############################
JENKINS_IP="192.168.56.15"
NEXUS_IP="192.168.56.16"
SONAR_IP="192.168.56.17"
BACKEND_IP="192.168.56.19"
TOMCAT_STAGING_IP="192.168.56.20" # port 8080
TOMCAT_STAGING2_IP="192.168.56.21" # port 8080
TOMCAT_PRODUCTION_IP="192.168.56.22" # port 8080
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "nexus", autostart:true do |nexus|
    ...
    nexus.vm.provision "shell", path: "nexus-setup-ubuntu.sh"
  ...
  config.vm.define "backend", autostart:false do |backend|
    ...
    backend.vm.provision "shell", inline: <<-SHELL
      sudo dnf update -y
    SHELL
    backend.vm.provision "shell", path: "setup-mariadb.sh"
    backend.vm.provision "shell", path: "setup-memcached.sh"
    backend.vm.provision "shell", path: "setup-rabbitmq.sh"
  end
  ...
  config.vm.define "staging", autostart:false do |staging|
    ...
    SHELL
    staging.vm.provision "shell", path: "tomcat-setup-ubuntu.sh"
  end

  ###################################################
  ###       TOMCAT STAGING  - ANSIBLE SETUP      #### 
  ###################################################
  config.vm.define "staging2", autostart:false do |staging2|
    staging2.vm.box = UBUNTU_VM
    staging2.vm.hostname = 'staging2'
    staging2.vm.network "private_network", ip: TOMCAT_STAGING2_IP
    staging2.vm.provider PROVIDER do |vbox|
      vbox.memory = "4096"
      vbox.cpus = "2"
    end
    #staging.vm.provision "shell", inline: <<-SHELL
